...
const { data: services } = await useAsyncData('services', () =>
  queryContent('szolgaltatasok')
    .only(['title', 'short', '_path', '_file', 'imageSrc', 'bgImageSrc'])
    .find()
)
...
<template>
  <div class="grid-4">
    <NuxtLink
      ...
      :key="service._path"
      :to="`/szolgaltatasok/${service._file
        .split('/')
        .pop()
        .replace('.md', '')}`"
    ...
    >
      <div
        ...
        :style="{
          backgroundImage: `url(${getFullImagePath(service.bgImageSrc)})`,
        }"
      >
        <NuxtImg
